...
M.open_data = function(tree, node, directory, save_tree, opts)
    ...
    if data == nil then
        ...
        data =
            mind_data.new_data_file(directory, node.contents[1].text, opts.edit.data_extension, contents, should_expand)
...
M.select_node_path = function(tree, opts)
    mind_ui.with_input('Path: /', nil, function(input)
        local parent, node = mind_node.get_node_by_path(tree, '/' .. input, opts.tree.automatic_creation)
...
M.move_node = function(tree, source_parent, source_node, target_parent, target_node, dir, opts)
    ...
    if source_parent == target_parent then
        ...
        if dir == mind_node.MoveDir.BELOW then
            mind_node.move_source_target_same_tree(source_parent, source_i, target_i + 1)
